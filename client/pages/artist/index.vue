<template>
  <div>
    ...
    <section v-if="filteredList.length > 0" id="artist-list" class="grid grid-cols-2 ms:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 1xl:grid-cols-7 2xl:grid-cols-10 gap-5 w-full justify-center p-5">
      <ArtistCard class="animate__animated animate__fadeIn" v-for="artist in filteredList" :key="artist.id" :artist="artist"/>
    </section>
...
<script>
  ...
  export default {
    ...
    methods:{
      async fetchData() {
        let artTmp = []
        console.log("Search", `https://comeback-api.herokuapp.com/artists/fulllimited?sortby=name&name=%${this.search}%&op=ilike&limit=20&offset=${this.maxArtist}`)
