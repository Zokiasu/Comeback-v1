<template>
	<div class="p-5 lg:p-10 space-y-5 text-white">
		...
		<section class="space-y-5 px-3">
			<section
				id="img-media-streaming-group"
				class="flex flex-col space-y-5 lg:flex-row lg:space-x-5 lg:space-y-0 relative"
			>
				<div id="image">
					<img
						class="w-full lg:w-96 aspect-video object-cover"
						:src="artist.image"
						:alt="artist.name"
					/>
				</div>
				<div id="media-streaming-group" class="flex flex-col space-y-3">
					<div id="social-media" class="space-y-1" v-if="artist.socials.length">
						...
						<div id="link-social" class="flex flex-wrap space-x-2">
							<LinkImg
								v-for="(media, index) in artist.socials"
								:key="index"
								:url="media"
								:name="media"
							/>
					...
					>
						...
						<div id="link-streaming" class="flex flex-wrap space-x-2">
							<LinkImg
								v-for="(stream, index) in artist.platforms"
								:key="index"
								:url="stream"
								:name="stream"
							/>
					...
					<div id="style" class="space-y-2" v-if="artist.styles.length">
						<h3 class="text-white text-xl">Styles</h3>
						<div class="space-x-1">
							<span
								v-for="(style, index) in artist.styles"
								:key="index"
								class="bg-gray-500 text-white p-1 px-2 rounded"
								>{{ style.name }}</span
							>
						</div>
					</div>
				...
				<div
					v-if="userInfo != null && displayLike"
					id="button"
					class="absolute lg:right-0 top-0"
				>
					<button
						@click="liked ? unfollowArtist() : followArtist()"
						:class="liked ? '' : ''"
						class="flex space-x-0.5 px-2 focus:outline-none rounded transition duration-300 ease-in-out transform hover:-translate-y-1 hover:scale-110 hover:font-bold"
					>
						<svg
							v-if="!liked"
			...
			<section class="space-y-5">
				...
				<div id="member" class="space-y-2" v-if="memberslist.length">
					...
					<div
						class="text-white grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8 gap-5"
					>
						<NuxtLink
							:to="`/artist/${members.id}`"
							class="p-3 rounded flex flex-col justify-center"
							v-for="(members, index) in memberslist"
							:key="index"
						>
							<div class="mb-0.5">
								<img
									class="Card rounded-full h-10 lg:h-20 w-10 lg:w-20 object-cover mx-auto"
									:src="members.image"
									alt=""
								/>
							</div>
							<span class="w-full h-full text-center xl:text-xl">
								{{ members.name }}
							</span>
						</NuxtLink>
					</div>
				</div>
				<div id="subunit" class="space-y-2" v-if="subunitlist.length">
					...
					>
						<NuxtLink
							:to="`/artist/${member.id}`"
							class="Card p-3 rounded flex flex-col justify-center"
							v-for="(member, index) in subunitlist"
							:key="index"
						>
							<div class="mb-0.5">
								<img
									class="rounded h-20 object-cover mx-auto"
									:src="member.image"
									alt=""
								/>
							</div>
				...
				<div id="group-unit" class="space-y-1" v-if="artist.groups.length">
					...
					<div class="flex flex-row">
						...
						>
							<div class="mb-0.5">
								<img
									class="rounded w-20 h-20 object-cover mx-auto"
									:src="group.image"
									alt=""
								/>
							</div>
				...
				>
					...
					<div id="release-list" class="flex flex-wrap gap-1">
						<ReleaseCard
...
export default {
	...
	async asyncData({ $axios, params }) {
		let artist = await $axios.$get(
			`https://comeback-api.herokuapp.com/artists/${params.id}`
		);
		...
		let memberslist = await $axios.$get(
			`https://comeback-api.herokuapp.com/artists/${params.id}/members?sortby=name`
		);
	...
	mounted() {
		...
		if (this.userInfo) {
			...
			this.$fire.auth.onAuthStateChanged(async function (users) {
				if (users != null) {
					that.displayLike = true;
					await that.setStoreData(users.uid);
					that.artist.followers.forEach((element) => {
	...
	methods: {
		...mapMutations(["SET_DATA_USER", "SET_TOKEN_USER"]),
		...
		async setStoreData(userId) {
			...
			await this.$axios
				.get(`https://comeback-api.herokuapp.com/users/${userId}`)
				.then((res) => {
