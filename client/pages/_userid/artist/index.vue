<template>
  <div>
    <section id="searchbar" class="my-8 mx-10 mb-14 flex w-full justify-start">
      ...
      <input id="search-input" type="text" placeholder="Search" v-model="search" class="w-1/2 pl-2 focus:outline-none rounded-r rounded-none bg-select-leftbar text-white placeholder-white">
    </section>
    <section id="artist-list" class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 3xl:grid-cols-6 px-10 gap-y-10 w-full justify-center my-10">
      <ArtistCard 
    ...
    computed: {
      filteredList() {
        return this.artists.filter(artist => {
          if(this.search != '') return (artist.name.toLowerCase().includes(this.search.toLowerCase()))
          return true
        })
      }
    },
